...
事务实现的原理就是使用了AOP，一旦使用了事务，那么就会给当前类创建一个代理对象，在方法执行前开启事务，如果正常结束就提交事务，没有正常结束就会回滚
...
4. 方法内部调用：
   ...
   > 由于事务内部自调用，调用的是被代理对象中没有AOP被增强的原始方法，也就是直接使用this调用，所以事务会失效
   >
   > 解决办法就是将内部调用的方法分离出来形成一个新的类，或者使用AopContext获取到被AOP增强的对象，调用被AOP增强的方法

5. 使用了@Transactional的类没有被spring管理，从而导致不会产生AOP，也就不会有事务

6. 数据库底层不支持事务，利用mysql的myisam就不支持事务

7. 设置了错误的传播属性导致不回滚

8. rollbackFor设置的异常属性有问题，导致不回滚
