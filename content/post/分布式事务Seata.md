...
XA是规范，目前主流数据库都实现了这种规范，实现的原理都是基于两阶段提交。相当于基于数据库**本身**的特性来实现分布式事务
...
1）修改application.yml文件（每个参与事务的微服务），开启XA模式，相当于存在分布式事务，一旦由数据库的请求就会被拦截，然后由RM统一管理，事务的提交和回滚分阶段进行：
...
2）给发起全局事务的**入口方法**添加@GlobalTransactional注解，这样才知道全局事务从哪里开始，之后整个方法的调用链都由seata监控:
