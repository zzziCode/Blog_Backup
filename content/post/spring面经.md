...
5. applicationContext在容器创建时不管是否用到都创建，后续用到的时候直接拿，更加快捷
...
1. 用@bean修饰方法，方法内**手动new**，可以干预实例化过程
2. 用@import直接导入类，无法干预实例化过程
   1. 用import导入importselector：返回一个字符串数组
   2. 用import导入importBeandefinitionregistrar：手动编写beanDefinition再完成实例化
...
2. **特殊**情况：出现**循环依赖**时，有必要的话会进行**提前AOP**，因为要确保属性注入时注入的是尝试aop之后的类
...
> 主要是`AOP`（面向切面编程）+数据库自带的事务，步骤类似于开启了一个**环绕通知**
...
3. 多线程无法保证事务一致性
4. 但是可以通过**编程式事务**，自己控制事务的提交，或者使用**分布式事务**，将多个线程的事务统一交给TC事务协调者管理，主流的有XA和AT两种模式：
...
1. 外部框架使用spring的**对外扩展点**想办法将自己的内部的bean交给spring
...
2. 具体是根据一个**处理器映射器**来判断请求映射到哪个方法中，没找到映射会报404
3. 每个controller都是一个bean，映射方法时会从子容器中找
4. 请求通过拦截器之后，请求传递到后端对应的方法上 ，执行对应的接口方法
5. 如果请求中有参数，就会将参数转换成java能够处理的格式
6. 后端方法执行处理之后，返回数据，此时数据可能会转换成json格式
7. 将响应的数据解析成视图
8. 将渲染好的视图响应给前端得到结果
...
2. 其内部有一个**@Import** 注解，导入的**DeferImportSelector**类实现了ImportSelector接口，这个类应该是延迟加载的，先等项目中的bean加载完了再加载，因为有一个ConditionalOnxxx注解**，保证需要的bean肯定被加载过后再自动配置**
...
> 打包时会将依赖的文件都打包到一起，相当于**本身自带环境**

<img src="C:/Users/zzzi/AppData/Roaming/Typora/typora-user-images/image-20240514194641183.png" alt="image-20240514194641183" style="zoom: 50%;" />

<img src="C:/Users/zzzi/AppData/Roaming/Typora/typora-user-images/image-20240514194955371.png" alt="image-20240514194955371" style="zoom: 50%;" />

1. 内部有一个maven-plugin插件可以将程序打包成 可运行的jar包，否则会报**没有主清单错误**
...
> run方法：启动项目，主要是形成IOC容器

1. 之后运行启动类中的run方法，内部读取环境信息，配置信息
2. 创建一个springApplication上下文并初始化，主要是调用refresh方法
...
1. 在自动配置类中定义当前的项目如何整合到spring中，利用spring的扩展点
2. 在自定义的starter中新建一个META-INF文件夹，内部存放一个spring.factories文件
3. 里面的内容存放的键值对，主要是当前**自动配置类**的类名，让spring能够感知到这个自动配置类
4. 自动配置类中定义当前程序中所需要的一些bean
5. 如果需要一些配置属性，还可以提供一个属性配置文件
6. 打包发布，项目中引入自定义的starter就可以引入当前框架

#### springboot读取配置文件的原理，配置文件加载顺序

> 核心就是事件监听的机制，或者说bean属性填充之前将占位符进行替换，介绍几种方法
