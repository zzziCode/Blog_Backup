...
1. fork一个子进程时，父子进程之间共享的是页表，这样减少了复制的时间
2. 写操作发生，会创建**副本**，父进程操作这个副本，子进程复制原来的旧数据
3. RDB快照保存完毕之后，里面的部分数据**不是最新**的
4. 因为在RDB快照保存时，父进程在副本上进行了修改
5. 这些修改只能在下次RDB中保存
6. **极端情况**所有内存中的数据都在RDB期间被修改，此时**都**需要创建副本，内存占用量是原来的两倍
